...
export default function Home({ progress, lang, course, totalCount, onTheme, onMix, onExam, onMock, onHard, onMarathon }) {
  const t = STR[lang]
  const themes = themesForCourse(course)
  const mastered = masteredCount(progress, course)
  ...
  return (
    <div className="home">
      ...
      <div className="extra-modes">
        <button className="mode-card mock" onClick={onMock}><span className="mode-title">{t.mode_mock}</span><span className="mode-sub">{t.mode_mock_sub}</span></button>
        <button className="mode-card hard" onClick={onHard}><span className="mode-title">{t.mode_hard}</span><span className="mode-sub">{t.mode_hard_sub}</span></button>
        <button className="mode-card marathon" onClick={onMarathon}><span className="mode-title">{t.mode_marathon}</span><span className="mode-sub">{t.mode_marathon_sub}</span></button>
      </div>
    </div>
  )
}
